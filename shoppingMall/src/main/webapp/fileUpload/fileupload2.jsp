<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<form action="fileupload2_process.jsp" method="post" enctype="multipart/form-data">
		<p>이름 : <input type="text" name = 'title1'>
		<p>제목 : <input type="text" name = 'subject1'>
		<p>파일 : <input type="file" name = "fileName1">
				
		<p>이름 : <input type="text" name = 'title2'>
		<p>제목 : <input type="text" name = 'subject2'>
		<p>파일 : <input type="file" name = "fileName2">
		
		<p>이름 : <input type="text" name = 'title3'>
		<p>제목 : <input type="text" name = 'subject3'>
		<p>파일 : <input type="file" name = "fileName3">
		
		<p> <input type="submit" value="파일 올리기">
	</form>
</body>
</html>